...
function yukle(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!dataSheets.length) { toast('Geçerli veri sayfası bulunamadı', 'err'); return; }
      ...
      toast('Dosya okunurken hata: ' + err.message, 'err');
      console.error(err);
    }
  };
...
function sutunHaritasiGoster(cols) {
  ...
  fields.forEach(function(f) {
    ...
    cols.forEach(function(c) {
      ...
      opt.textContent = c;
      // Otomatik tahmin
      if (colMap[f.key] === c) opt.selected = true;
      sel.appendChild(opt);
    });
    row.appendChild(lbl);
    row.appendChild(sel);
...
function exportCsv() {
  ...
  var lines = ['Sayfa,TİP,AY,Test Kaynaklı'].concat(
    rows.map(function(r){ return [r._sayfa,r.tip,r.ay,r.testKaynakli===null?'BELİRSİZ':r.testKaynakli?'EVET':'HAYIR'].map(csvVal).join(','); })
  );
